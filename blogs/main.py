...
@app.get('/blog', response_model=List[BlogResponse], status_code=status.HTTP_200_OK, tags=['blogs'])
def get_all_blogs(db: SessionLocal = Depends(get_db)):
    blogs = db.query(Blog).all()
    return blogs

@app.get('/blog/{blog_id}', response_model=BlogResponse, status_code=status.HTTP_200_OK, tags=['blogs'])
...
@app.put('/blog/{blog_id}', response_model=BlogResponse, status_code=status.HTTP_200_OK, tags=['blogs'])
...
@app.get('/user/{user_id}', response_model=UserResponse, status_code=status.HTTP_200_OK, tags=['users'])
...
@app.get('/user', response_model=List[UserResponse], status_code=status.HTTP_200_OK, tags=['users'])
...
@app.put('/user/{user_id}', response_model=UserResponse, status_code=status.HTTP_200_OK, tags=['users'])
